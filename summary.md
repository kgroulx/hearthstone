...
While these plots are rather colorful and messy, a Shiny app with interactivity to disentangle a bit more of these graphs as well as see the plots of card power by rarity can also be found in this folder under app.R.  Be sure to also download the file sortedMinions.csv from the folder as well so that it can run properly.
